...
<html>
	<head>
		...
		<script type="text/javascript">
			function doEdit(id){
                showModalDlg((id ? "编辑" : "新建") + "主界面", '<%= contextPath%>/layout/edit/' + id, 600, 250, function (s) {
                    doReset();
                });
			}
            ...
            function doDel(id) {
                ...
                $.post('<%= contextPath%>/layout/del/' + id, function (data) {
	  ...
	  <div id="divSearch" class="">
          <table class="paramTable" width="100%" border="1" cellpadding="0" cellspacing="0">
              <tr>
                  ...
                  <td><input id="p_textContent" class="paramIpt" name="textContent" type="text"/></td>
                  <td class="filedName">图片名称:</td>
                  <td><input id="p_picName" class="paramIpt" name="picName" type="text"/></td>
                  <td class="filedName">是否显示:</td>
                  <td>
                      <select id="p_useing" class="paramIpt" name="useing">
                          <option value=""></option>
		...
		<script type="text/javascript">
			var ajaxPage = new $AjaxPage("pageList", "<%= contextPath%>/ajaxpage/layout", 20);
            ajaxPage.afterRow(function (rowIndex, row, datas) {
                var layoutid = datas.get("id");
                row.cells[row.cells.length - 1].innerHTML = "<a href='javascript:void(0);' onclick=\"doEdit(" + layoutid + ");\">编辑</a>&nbsp;"
                       +  "<a href='javascript:void(0);' onclick=\"doDel(" + layoutid + ");\">删除</a>&nbsp;";
            });
